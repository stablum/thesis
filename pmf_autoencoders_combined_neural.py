...
def main():
    ...
    def make_net(input_var,in_dim,out_dim):
        input_var_reshaped = input_var.reshape((1, in_dim))
        l_in = lasagne.layers.InputLayer((1,in_dim),input_var=input_var_reshaped)
        l_fc = lasagne.layers.DenseLayer(l_in,out_dim,nonlinearity=g)
    ...
    def make_predict_to_1(ui,vj):
        o_ui,net_ui_params = make_net(ui,config.K,fc_dim)
        o_ui.name = "o_ui"
        o_vj,net_vj_params = make_net(vj,config.K,fc_dim)
        o_vj.name = "o_vj"
        combined = T.concatenate([o_ui,o_vj],axis=0)
        combined.name = "combined"
        prediction,net_combined_params = make_net(combined,2*config.K,1)
